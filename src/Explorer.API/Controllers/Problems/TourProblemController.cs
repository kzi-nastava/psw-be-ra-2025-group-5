...
namespace Explorer.API.Controllers.ProblemReporting
{
    ...
    public class TourProblemController : ControllerBase
    {
        ...
        public ActionResult<PagedResult<TourProblemDto>> GetAll([FromQuery] int page, [FromQuery] int pageSize)
        {
            ...
            if (isAdmin)
            {
                ...
                result.Results.ForEach(tp =>
                {
                    try
                    {
                        var tour = _tourService.Get(tp.TourId);
                        if (tour != null)
                        {
        ...
        public ActionResult<TourProblemDto> Create([FromBody] TourProblemDto tourProblem)
        {
            ...
            try
            {
                var tour = _tourService.Get(result.TourId);
        ...
        public ActionResult<List<CompletedToursDto>> GetCompletedTours(long id)
        {
            ...
            var completedTours = result.Select(execution =>
                ...
                {
                    Name = _tourService.Get(execution.TourId)?.Name ?? "Unknown",
